...
namespace Game.Network {
    public class SyncClassList<T> : IList<T>, IReadOnlyList<T>, SyncObject {
        public delegate void SyncClassChanged(Operation op, int itemIndex, T oldItem, T newItem);
        ...
        public event SyncClassChanged Callback;
        ...
        public SyncClassList() : this(EqualityComparer<T>.Default) {
        }

        public SyncClassList(IEqualityComparer<T> comparer) {
            this.comparer = comparer ?? EqualityComparer<T>.Default;
            objects = new List<T>();
        }

        public SyncClassList(IList<T> objects, IEqualityComparer<T> comparer = null) {
            this.comparer = comparer ?? EqualityComparer<T>.Default;
        ...
        public void AddOperation(Operation op, int itemIndex, T oldItem, T newItem) {
            if (IsReadOnly) {
                throw new InvalidOperationException("SyncClasss can only be modified at the server");
            }
        ...
        public void OnSerializeAll(NetworkWriter writer) {
            ...
            for (int i = 0; i < objects.Count; i++) {
                T obj = objects[i];
                string json = Functions.ObjectToStringJson(obj);
                writer.Write(json);
            }
            ...
            for (int i = 0; i < changes.Count; i++) {
                ...
                switch (change.operation) {
                    case Operation.OP_ADD:
                        string add_json = Functions.ObjectToStringJson(change.item);
                        writer.Write(add_json);
                        break;
                    ...
                    case Operation.OP_SET:
                        writer.WriteUInt32((uint)change.index);
                        string set_json = Functions.ObjectToStringJson(change.item);
                        writer.Write(set_json);
                        break;
                }
            }
        }
        ...
        public void OnDeserializeAll(NetworkReader reader) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                T obj = Functions.StringJsonToObject<T>(json);
                objects.Add(obj);
            }
        ...
        public void OnDeserializeDelta(NetworkReader reader) {
            ...
            for (int i = 0; i < changesCount; i++) {
                ...
                switch (operation) {
                    case Operation.OP_ADD:
                        string json_add = reader.ReadString();
                        newItem = Functions.StringJsonToObject<T>(json_add);
                        if (apply) {
                            index = objects.Count;
                            objects.Add(newItem);
                        }
                    ...
                    case Operation.OP_INSERT:
                        ...
                        string json_inset = reader.ReadString();
                        newItem = Functions.StringJsonToObject<T>(json_inset);
                        if (apply) {
                    ...
                    case Operation.OP_SET:
                        index = (int)reader.ReadUInt32();
                        string json_set = reader.ReadString();
                        newItem = Functions.StringJsonToObject<T>(json_set);
                        if (apply) {
                            oldItem = objects[index];
